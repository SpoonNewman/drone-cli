...
pub struct Drone<A: DroneApi> {
    api: A,
}

impl<A: DroneApi> Drone<A> {
    pub fn new(api: A) -> Self { Self { api } }

    pub async fn connect(&mut self) -> Result<()> { self.api.connect().await }
    pub async fn arm(&mut self) -> Result<()> { self.api.arm().await }
    pub async fn takeoff(&mut self, altitude_m: f32) -> Result<()> { self.api.takeoff(altitude_m).await }
    pub async fn land(&mut self) -> Result<()> { self.api.land().await }
    pub async fn status(&mut self) -> Result<TelemetrySnapshot> { self.api.status().await }
}
...
// // : Big Refactor: This code assumes that Drone service is going to use SBUS (serial) to interact with the Flight Controller. This is
...
// // : Followup Discussion: DroneState is confusing. Not sure whether we want to actually track the drone state or not although philosophically I
// // agree with this. 
...
// // : None of this can be used. See above statements. Make a new Drone service that is able to do GET/POST requests with and without headers
